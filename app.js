...
const firebaseConfig = {
  apiKey: "AIzaSyCTyJWkN4jbx1wPihmpGkX9h8jQTtCQ7Hk",
  authDomain: "reultwebsite.firebaseapp.com",
  databaseURL: "https://reultwebsite-default-rtdb.firebaseio.com",
  projectId: "reultwebsite",
  storageBucket: "reultwebsite.firebasestorage.app",
  messagingSenderId: "738432914117",
  appId: "1:738432914117:web:40b614a1731c827b87d6ac",
  measurementId: "G-BGDKQ4KKHH"
};

let db = null;
if (typeof firebase !== "undefined" && firebaseConfig.databaseURL && firebaseConfig.databaseURL !== "YOUR_FIREBASE_DATABASE_URL") {
...
async function loadData() {
  if (db) {
    try {
      const snapshot = await db.ref("result_board").once("value");
      ...
      console.warn("Firebase fetch failed, falling back to local files / localStorage", error);
    }
  }
  ...
  try {
    ...
    console.warn("Failed to fetch data.json, falling back to localStorage", error);
    const stored = localStorage.getItem(STORAGE_KEY);
    if (stored) {
      try {
        cachedData = JSON.parse(stored);
      } catch {
        cachedData = structuredClone(sampleData);
      }
    } else {
      cachedData = structuredClone(sampleData);
    }
...
function getData() {
  ...
  return normalizeData(cachedData);
}
...
async function saveData(data) {
  ...
  if (db) {
    try {
      await db.ref("result_board").set(data);
    } catch (error) {
  ...
  try {
    ...
    if (!response.ok) {
      console.warn("Local file save returned status: " + response.status);
    }
  } catch (error) {
    // Expected when running deployed on Vercel
  }
}
...
function updateNewsSlider() {
  const track = document.querySelector("#newsList");
  if (!track) return;
  const slides = track.querySelectorAll(".news-item");
  ...
  if (currentNewsSlide >= totalSlides) {
    currentNewsSlide = 0;
...
function renderPublicSite() {
  ...
  document.querySelector("#newsList").innerHTML = data.news.length
    ? data.news
      .slice()
      .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
      .map((item, index) => {
        const hasImage = !!item.imageUrl;
        const tagText = index === 0 ? "Live" : "Info";
        const tagClass = index === 0 ? "news-tag live" : "news-tag";
        return `
            <article class="news-item${hasImage ? " has-image" : ""}">
              ${hasImage ? `<img class="news-image" src="${escapeHtml(normalizePosterImageUrl(item.imageUrl))}" data-drive-id="${escapeHtml(extractGoogleDriveFileId(item.imageUrl))}" data-fallback-index="0" onerror="handlePosterImageError(this)" alt="">` : ""}
    ...
    : `<div class="empty">No news updates yet.</div>`;
...
document.addEventListener("DOMContentLoaded", async () => {
  if (document.querySelector("#programGrid")) {
    ...
    const dialog = document.querySelector("#posterDialog");
    document.querySelector("#closeDialog").addEventListener("click", closePosterDialog);
    ...
    if (db) {
      db.ref("result_board").on("value", (snapshot) => {
        const data = snapshot.val();
        if (data) {
          cachedData = data;
          renderPublicSite();
      ...
      // Fallback: poll data every 15 seconds from server
      setInterval(async () => {
        await loadData();
        renderPublicSite();
      }, 15000);
