...
// Replace these with your actual credentials from your Supabase Dashboard
const SUPABASE_URL = "https://hkoriuempveakfjoixtz.supabase.co";
const SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imhrb3JpdWVtcHZlYWtmam9peHR6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzkzODY3OTIsImV4cCI6MjA5NDk2Mjc5Mn0.opqwJbakbN6lgdccaMpI8TQ5RvUNcdF3qOlLxpF77Po";
...
async function loadData() {
  if (supabaseClient) {
    try {
      ...
      console.warn("Supabase fetch failed, falling back to local files / localStorage", error);
    }
  }
...
document.addEventListener("DOMContentLoaded", async () => {
  if (document.querySelector("#programGrid")) {
    ...
    // If Supabase is configured, subscribe to real-time updates
    if (supabaseClient) {
      supabaseClient
        .channel("public:result_board")
        .on(
          "postgres_changes",
          { event: "UPDATE", schema: "public", table: "result_board", filter: "id=eq.1" },
          (payload) => {
            if (payload.new && payload.new.data) {
              cachedData = payload.new.data;
              renderPublicSite();
            }
          }
        )
